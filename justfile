...
lc-contract-profiling-sepolia:
    #!/usr/bin/env bash
    set -euxo pipefail
    forge script contracts/test/DeployLightClientTestScript.s.sol --sig "runBench(uint32 numBlocksPerEpoch, uint64 numInitValidators, uint32 maxHistorySeconds)" {{NUM_BLOCKS_PER_EPOCH}} {{NUM_INIT_VALIDATORS}} {{MAX_HISTORY_SECONDS}} --fork-url ${SEPOLIA_RPC_URL} --broadcast --verify --etherscan-api-key ${ETHERSCAN_API_KEY} --chain-id sepolia
