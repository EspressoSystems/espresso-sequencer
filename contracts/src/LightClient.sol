...
contract LightClient is Initializable, OwnableUpgradeable, UUPSUpgradeable {
  ...
  function getVersion() public pure virtual returns (uint8 majorVersion, uint8 minorVersion, uint8 patchVersion) {
    return (1, 0, 0);
  }
  ...
  function _initializeState(
    ...
    if (
      ...
      _stateHistoryRetentionPeriod < 1 hours ||
  ...
  function newFinalizedState(
    ...
    if (newState.viewNum <= finalizedState.viewNum || newState.blockHeight <= finalizedState.blockHeight) {
  ...
  function verifyProof(LightClientState memory state, IPlonkVerifier.PlonkProof memory proof) internal virtual {
  ...
  function updateStateHistory(uint64 blockNumber, uint64 blockTimestamp, LightClientState memory state) internal {
    if (
      stateHistoryCommitments.length != 0 &&
      blockTimestamp - stateHistoryCommitments[stateHistoryFirstIndex].l1BlockTimestamp > stateHistoryRetentionPeriod
    ) {
    ...
    stateHistoryCommitments.push(
      StateHistoryCommitment(blockNumber, blockTimestamp, state.blockHeight, state.blockCommRoot)
    );
  }
  ...
  function lagOverEscapeHatchThreshold(uint256 blockNumber, uint256 blockThreshold) public view virtual returns (bool) {
  ...
  function getHotShotCommitment(
    uint256 hotShotBlockHeight
  ) public view virtual returns (BN254.ScalarField hotShotBlockCommRoot, uint64 hotshotBlockHeight) {
    ...
    for (uint256 i = stateHistoryFirstIndex; i < commitmentsHeight; i++) {
      ...
      if (stateHistoryCommitments[i].hotShotBlockHeight >= hotShotBlockHeight) {
        return (stateHistoryCommitments[i].hotShotBlockCommRoot, stateHistoryCommitments[i].hotShotBlockHeight);
      }
    }
  }
  ...
  function setstateHistoryRetentionPeriod(uint32 historySeconds) public onlyOwner {
    if (historySeconds < 1 hours || historySeconds > 365 days || historySeconds <= stateHistoryRetentionPeriod) {
