...
async function main() {
    try {
        ...
        await proposeSetProverTransaction(safeSdk, safeService, await orchestratorSigner.getAddress(), safeAddress, permissionedProverAddress);
        
        console.log(`The other owners of the Safe Multisig wallet need to sign the transaction via the Safe UI https://app.safe.global/transactions/queue?safe=sep:${safeAddress}`);
    } catch (error) {
...
function createPermissionedProverTxData(proverAddress: string): string{
    ...
    const abi = [
        "function setPermissionedProver(address)"
    ];
 ...
 */
async function createSafeTransaction(safeSDK: Safe, contractAddress: string, data: string, value: string): Promise<any> {
