...
use hotshot_types::traits::{node_implementation::Versions, signature_key::BuilderSignatureKey};
use tide_disco::App;
use url::Url;
...
/// effectively.
pub async fn run_standalone_service<V: Versions>(options: Options, ver: V) {
    ...
    let _cdn_tasks = if let Some(cdn_broker_url_string) = options.cdn_marshal_endpoint() {
        ...
        let cdn_receive_message_task = CdnReceiveMessagesTask::new(cdn_network, url_sender, ver);
        let broadcast_roll_call_task =
            BroadcastRollCallTask::new(broadcast_cdn_network, public_key, ver);
