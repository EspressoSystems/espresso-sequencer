...
use contract_bindings::{
    ...
    hot_shot::HotShot,
    light_client::LIGHTCLIENT_ABI,
    light_client_mock::{LightClientMock, LIGHTCLIENTMOCK_ABI},
...
async fn deploy_light_client_contract<M: Middleware + 'static>(
    ...
    if use_mock_contract {
        deploy_mock_light_client_contract(l1, plonk_verifier, vk).await
    } else {
        deploy_production_light_client_contract(l1, plonk_verifier, vk).await
    }
}

async fn deploy_production_light_client_contract<M: Middleware + 'static>(
    l1: Arc<M>,
...
async fn deploy_mock_light_client_contract<M: Middleware + 'static>(
    l1: Arc<M>,
    plonk_verifier: Address,
    vk: Address,
) -> anyhow::Result<Address> {
    ...
    let dummy_genesis: LightClientState = ParsedLightClientState::dummy_genesis().into();
    let contract = light_client_factory
        .deploy((dummy_genesis, u32::MAX))?
        .send()
        .await?;
    Ok(contract.address())
}
