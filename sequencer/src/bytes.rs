...
// Re-export `bytes![]` macro, which gets exported at the crate root by default.
pub use crate::bytes;
...
mod test {
    ...
    fn test_bytes_serde() {
        ...
        for len in [0, 1, 10, 1000] {
            ...
            assert_eq!(bytes, bincode::deserialize::<Bytes>(&binary).unwrap());
            ...
            // Check deserialization.
            assert_eq!(bytes, serde_json::from_value::<Bytes>(json).unwrap());
        }
    }
}
