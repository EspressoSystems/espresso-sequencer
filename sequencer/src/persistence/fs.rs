...
impl Inner {
    ...
    fn collect_garbage(&mut self, prune_view: ViewNumber) -> anyhow::Result<()> {
        self.prune_files(self.da_dir_path(), prune_view)?;
        self.prune_files(self.vid_dir_path(), prune_view)?;
        self.prune_files(self.quorum_proposals_dir_path(), prune_view)?;

        // Save the most recent leaf as it will be our anchor point if the node restarts.
        if prune_view > ViewNumber::genesis() {
            self.prune_files(self.decided_leaf_path(), prune_view - 1)?;
        }

        Ok(())
    }

    fn prune_files(&mut self, dir_path: PathBuf, prune_view: ViewNumber) -> anyhow::Result<()> {
        if !dir_path.is_dir() {
            return Ok(());
        }
...
mod test {
    ...
    async fn test_load_quorum_proposals_invalid_extension() {
        ...
        let leaf = Leaf::genesis(&Default::default(), &NodeState::mock()).await;
    ...
    async fn test_load_quorum_proposals_malformed_data() {
        ...
        let leaf: Leaf = Leaf::genesis(&Default::default(), &NodeState::mock()).await;
        let privkey = PubKey::generated_from_seed_indexed([0; 32], 1).1;
        let signature = PubKey::sign(&privkey, &[]).unwrap();
